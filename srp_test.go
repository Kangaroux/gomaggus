...
func Test_calcServerSessionKey(t *testing.T) {
	...
	for _, tc := range testCases {
		clientPublicKey := ReverseBytes(mustDecodeHex(tc.clientPublicKey))
		verifier := ReverseBytes(mustDecodeHex(tc.verifier))
		serverPrivateKey := ReverseBytes(mustDecodeHex(tc.serverPrivateKey))
		serverPublicKey := ReverseBytes(calcServerPublicKey(ReverseBytes(verifier), ReverseBytes(serverPrivateKey)))
		expected := ReverseBytes(mustDecodeHex(tc.expected))
